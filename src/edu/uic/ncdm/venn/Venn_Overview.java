...
public class Venn_Overview{
   ...
   public boolean[] isActive;
	...
	public void compute() {
		numArt = main.ProvenanceMatrix_1_7.artStrings.length;
		...
		for (int i =0; i< srcTaxonomy.size(); i++){
			for (int j =0; j< trgTaxonomy.size(); j++){
				...
				if (articulations[i][j].size()>1){
					...
					//	continue;
					//}	
					String str = main.ProvenanceMatrix_1_7.artStrings[(Integer) articulations[i][j].get(0)];
					for (int k =1; k< articulations[i][j].size(); k++){
						//if ((Integer) articulations[i][j].get(k)==4)
						//	continue;
						str += "&"+main.ProvenanceMatrix_1_7.artStrings[(Integer) articulations[i][j].get(k)];
					}
		...
		for (int i =0; i< numArt; i++){
			data[i][0] = main.ProvenanceMatrix_1_7.artStrings[i];
			areas[i]=count[i];
		}
	...
	public void draw(float xPanelRight, float yy4) {
        ...
        for (int i = centers.length-1; i >=0; i--) {
            ...
            if (radius>0){
            	Color color = new Color(main.ProvenanceMatrix_1_7.mappingColorRelations[i]);  
	...
	public boolean mouseClicked() {
		if (brushing>=0){
			 main.ProvenanceMatrix_1_7.currentRelation = brushing;
			 isActive[brushing] = !isActive[brushing];
			 return true;
		}
		else{
			main.ProvenanceMatrix_1_7.currentRelation = -2;
			return false;
		}	
	}
